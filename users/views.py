...
def github_login(request):
    ...
    redirect_uri = "http://airbnb-clone.eba-k2kqqtm3.ap-northeast-2.elasticbeanstalk.com/users/login/github/callback"
    return redirect(
    ...
    try:
        ...
        if code is not None:
            ...
            if error is not None:
                raise GithubException("액세스 토큰을 가질 수 없습니다.")
            else:
                ...
                if username is not None:
                    ...
                    try:
                        ...
                        if user.login_method != models.User.LOGIN_GITHUB:
                            raise GithubException(f"{user.login_method}로 로그인해주십시오.")
                    except models.User.DoesNotExist:
                    ...
                    raise GithubException("프로필을 가질 수 없습니다.")
        else:
            raise GithubException("코드를 가질 수 없습니다.")
...
def kakao_login(request):
    ...
    redirect_uri = "http://airbnb-clone.eba-k2kqqtm3.ap-northeast-2.elasticbeanstalk.com/users/login/kakao/callback"
    return redirect(
...
def kakao_callback(request):
    try:
        ...
        error = token_json.get("error", None)
        if error is not None:
            raise KakaoException("권한부여 코드를 가질 수 없습니다.")
        access_token = token_json.get("access_token")
        profile_request = requests.get(
        ...
        if email is None:
            raise KakaoException("이메일주소를 확보하지 못했습니다.")
        ...
        try:
            ...
            if user.login_method != models.User.LOGING_KAKAO:
                raise KakaoException(f"{user.login_method}로 로그인하여 주십시오.")
        except models.User.DoesNotExist:
            user = models.User.objects.create(
                email=email,
                username=email,
